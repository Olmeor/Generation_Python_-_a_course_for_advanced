...
with open('goats.txt', 'r', encoding='utf-8') as goat, open('answer.txt', 'w', encoding='utf-8') as answer:
    arr = list(goat.readlines())
    arr = arr[arr.index('GOATS\n') + 1:]
    res = {}
    for g in arr:
        res[g] = res.get(g, 0) + 1
    for k, v in res.items():
        if round(v / sum(res.values()), 2) > 0.07:
            print(k, end="", file=answer)
